...
import { Twitter, Linkedin, ExternalLink, Clock } from 'lucide-react';
...
const InstructorDetailPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      const inst = await getInstructorById(instructorId);
      if (inst) {
        setInstructor(inst);
        const instructorCourses = await getCoursesByAuthorName(inst.name);
  ...
  if (!instructor) {
    return (
      <div className="bg-white pt-40 pb-20 text-center">
        ...
        <Link to="/academy/instructors" className="mt-8 inline-block text-blue-600 hover:underline">&larr; Kembali</Link>
      </div>
    );
  }
    ...
    <div className="bg-slate-50 min-h-screen pt-32 pb-20">
      <div className="container mx-auto px-4 max-w-5xl">
        <header className="bg-white rounded-3xl shadow-lg p-8 flex flex-col md:flex-row items-center gap-8 mb-12">
          <img src={instructor.image} alt={instructor.name} className="w-40 h-40 rounded-full object-cover border-4 border-blue-200 shadow-md" />
          <div className="text-center md:text-left">
            ...
            <div className="flex items-center justify-center md:justify-start gap-4 mt-6">
              ...
              <a href={instructor.portfolio} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-semibold text-slate-600 hover:text-blue-600"><ExternalLink className="w-4 h-4 mr-1" /> Portofolio</a>
            </div>
          </div>
        </header>
                ...
                <Link to={course.href} key={course.id} className="block bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-blue-200 transition-all duration-300 hover:-translate-y-2 group">
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800`}>{course.level}</span>
                      <div className="flex items-center gap-1"><StarRating rating={course.score} /><span className="text-sm font-semibold text-slate-700 ml-1">{course.score}</span></div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100 text-sm text-slate-500">
                      ...
                      <div className="flex items-center gap-1">{course.students} Siswa</div>
                    </div>
                  </div>
                </Link>
              ))}

...
const Header = ({ isDropdownVisible, onDropdownToggle }) => {
  ...
  useLayoutEffect(() => {
    ...
    tl.set(headerRef.current, { y: -100, opacity: 0 })
  ...
  const handleMouseEnter = useCallback((submenuContent) => {
  ...
  const headerClasses = useMemo(() => {
    const baseClasses = 'w-full fixed top-0 left-0 z-50 transition-all duration-500 ease-out';
    ...
    <>
      ...
      >
            ...
            <nav className="hidden lg:flex absolute inset-x-0 items-center justify-center space-x-1">
              {navItems.map((item, idx) => (
                <div
                  ...
                  onMouseEnter={() => handleMouseEnter(item.type === 'mega-menu' ? item.megaMenuContent : null)}
                >
                  <a 
                    href={item.href || '#'}
        ...
        <div
          ...
          style={{ visibility: 'hidden', opacity: 0, transform: 'translateY(-20px) scale(0.95)' }}
          onMouseEnter={() => {
            if (timeoutRef.current) {
              clearTimeout(timeoutRef.current);
              timeoutRef.current = null;
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                  ...
                  <div className="lg:col-span-1 bg-gradient-to-br from-blue-50 to-purple-50 p-8 rounded-2xl border border-blue-100/50">
                    <h4 className="text-lg font-bold text-gray-900 mb-6">{activeSubmenu.whatsNew?.title}</h4>
                    <ul className="space-y-3 mb-8">
                      {activeSubmenu.whatsNew?.items.map((item, i) => (
                        <li key={i} className="mega-menu-item flex items-start text-sm text-gray-700">
      ...
      {/* Mobile Menu */}
      <div 
        className={`lg:hidden fixed inset-0 z-40 transform transition-all duration-500 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        role="dialog"
        aria-modal="true"
                  ...
                  <div key={idx} className="space-y-4">
                    ...
                    {item.megaMenuContent && (
                      <div className="ml-12 space-y-3">
                        {[...item.megaMenuContent.featuredCards, ...item.megaMenuContent.smallCards].map((card, i) => (
                          <a
                            key={i}
                            href={card.href}

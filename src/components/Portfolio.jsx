...
const Portfolio = () => {
  ...
  const handleCTAClick = () => {
    ...
    gsap.to(sectionRef.current, {
      ...
      onComplete: () => {
        ...
        gsap.set(sectionRef.current, { clearProps: 'all', autoAlpha: 1, yPercent: 0 });
      },
    });
  };
  ...
  useEffect(() => {
    ...
    const updateActive = () => {
      ...
      setActiveIndex(best);
      cards.forEach((el, i) => {
        gsap.to(el, { scale: i === best ? 1 : 0.92, opacity: i === best ? 1 : 0.6, duration: 0.25, ease: 'power2.out' });
      });
    };
  ...
  useEffect(() => {
    ...
    gsap.fromTo(descRef.current, { autoAlpha: 0, y: 8 }, { autoAlpha: 1, y: 0, duration: 0.3, ease: 'power2.out' });
  }, [activeIndex]);
  ...
  return (
    <section ref={sectionRef} className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-10">
          <p className="text-gray-500 text-xs tracking-widest uppercase">Previous Work</p>
        </div>

        {/* Cards row */}
        <div ref={rowRef} className="flex gap-8 overflow-x-auto pb-4 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {projects.map((p, i) => (
            <div
              key={i}
              data-portfolio-card
              className="snap-start will-change-transform flex-shrink-0"
              onClick={p.cta ? handleCTAClick : undefined}
              role={p.cta ? 'button' : undefined}
            >
              <PortfolioCard {...p} variant="light" />
            </div>
          ))}
        </div>
        ...
        <div ref={descRef} className="mt-4 min-h-[56px]">
          {projects[activeIndex] && !projects[activeIndex].cta && (
            <div className="text-gray-900">
              <div className="text-xs tracking-widest uppercase text-gray-500">{projects[activeIndex].partnerLabel}</div>
              <div className="font-extrabold">{projects[activeIndex].title} <span className="text-gray-700">{projects[activeIndex].subtitle}</span></div>
              <div className="text-xs text-gray-500">{projects[activeIndex].tag}</div>
            </div>
          )}
        </div>
      </div>

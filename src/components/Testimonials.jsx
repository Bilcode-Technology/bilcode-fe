...
class AnimationEngine {
  ...
  animateCardsSequence(topRowElement, bottomRowElement) {
    ...
    if (topRowElement) {
      ...
      topRowElement.style.animation = 'slideInFromRight 2s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards';
    ...
    if (bottomRowElement) {
      setTimeout(() => {
        ...
        bottomRowElement.style.animation = 'slideInFromLeft 2s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards';
  ...
  createInfiniteScroll(element, options = {}) {
    ...
    const keyframes = `
      @keyframes infiniteScroll${options.direction === 'reverse' ? 'Reverse' : ''} {
        0% { transform: translateX(${options.direction === 'reverse' ? '-' + totalWidth + 'px' : '0'}); }
        100% { transform: translateX(${options.direction === 'reverse' ? '0' : '-' + totalWidth + 'px'}); }
      }
    `;
    ...
    container.style.animation = `infiniteScroll${options.direction === 'reverse' ? 'Reverse' : ''} ${duration}s linear infinite`;
      ...
      >
        <div className={`relative w-full h-full bg-gradient-to-br ${bgColor} overflow-hidden`}>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`relative bg-white/95 backdrop-blur-sm rounded-full p-6 transition-all duration-500 shadow-lg ${ 
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center mt-8">
            ...
            <div className="ml-4 relative">
              ...
              <p className={`text-sm ${textColor === "text-gray-800" ? "text-gray-600" : "text-white/80"}`}>{location}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute z-10 text-center pointer-events-none select-none">
        ...
        <h1 className="title-text text-[6rem] md:text-[10rem] font-black text-gray-800 tracking-tighter bg-gradient-to-r from-gray-800 via-gray-900 to-gray-700 bg-clip-text text-transparent drop-shadow-sm">STORIES</h1>
        <h1 className="title-text text-[4rem] md:text-[6rem] font-bold text-gray-200 tracking-wider">SHARED</h1>
      </div>
